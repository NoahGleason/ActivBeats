...
//	private File file;						// File that will be read from or written to
	...
	public static WavFile newWavFile(OutputStream outputStream, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
	...
	public static WavFile openWavFile(InputStream iS) throws IOException, WavFileException
	{
	...
	public static void writeRaw(WavFile wav, List<Double> data) throws IOException, WavFileException {
		...
		while (frameCounter < wav.getNumFrames())
		{
...
//				}
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				WavFile readWavFile = openWavFile(new FileInputStream(new File(filename)));
				readWavFile.display();
				...
				WavFile writeWavFile = newWavFile(new FileOutputStream(new File("out.wav")), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
			...
			WavFile writeWavFile = newWavFile(new FileOutputStream(new File("out2.wav")), 1, 10, 23, 44100);
